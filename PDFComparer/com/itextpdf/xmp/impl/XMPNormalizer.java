...
/*     */ 
/* 155 */     for (Iterator it = xmp.getRoot().iterateChildren(); it.hasNext(); )
/*     */     {
/* 157 */       XMPNode currSchema = (XMPNode)it.next();
/* 158 */       if ("http://purl.org/dc/elements/1.1/".equals(currSchema.getName()))
/*     */       {
/* 160 */         normalizeDCArrays(currSchema);
/*     */       }
/* 162 */       else if ("http://ns.adobe.com/exif/1.0/".equals(currSchema.getName()))
/*     */       {
...
/*     */         }
/*     */       }
/* 173 */       else if ("http://ns.adobe.com/xmp/1.0/DynamicMedia/".equals(currSchema.getName()))
/*     */       {
...
/*     */         }
/*     */       }
/* 184 */       else if ("http://ns.adobe.com/xap/1.0/rights/".equals(currSchema.getName()))
/*     */       {
...
/*     */     throws XMPException
/*     */   {
/* 265 */     if ((arrayNode == null) || (!arrayNode.getOptions().isArray()))
/*     */     {
/* 269 */       return;
/*     */     }
/*     */ 
...
/*     */ 
/* 275 */     for (Iterator it = arrayNode.iterateChildren(); it.hasNext(); )
/*     */     {
/* 277 */       XMPNode currChild = (XMPNode)it.next();
/* 278 */       if (currChild.getOptions().isCompositeProperty())
/*     */       {
/* 281 */         it.remove();
/*     */       }
/* 283 */       else if (!currChild.getOptions().getHasLanguage())
/*     */       {
...
/*     */ 
/* 322 */     for (Iterator schemaIt = tree.getUnmodifiableChildren().iterator(); schemaIt.hasNext(); )
/*     */     {
/* 324 */       XMPNode currSchema = (XMPNode)schemaIt.next();
/* 325 */       if (currSchema.getHasAliases())
/*     */       {
/* 330 */         for (Iterator propertyIt = currSchema.iterateChildren(); propertyIt.hasNext(); )
/*     */         {
/* 332 */           XMPNode currProp = (XMPNode)propertyIt.next();
/*     */ 
/* 334 */           if (currProp.isAlias())
/*     */           {
/* 339 */             currProp.setAlias(false);
/*     */ 
/* 342 */             XMPAliasInfo info = XMPMetaFactory.getSchemaRegistry().findAlias(currProp.getName());
/*     */ 
/* 344 */             if (info != null)
/*     */             {
/* 347 */               XMPNode baseSchema = XMPNodeUtils.findSchemaNode(tree, info.getNamespace(), null, true);
/*     */ 
/* 349 */               baseSchema.setImplicit(false);
/*     */ 
/* 351 */               XMPNode baseNode = XMPNodeUtils.findChildNode(baseSchema, info.getPrefix() + info.getPropName(), false);
/*     */ 
...
/*     */                 else
/*     */                 {
/* 370 */                   baseNode = new XMPNode(info.getPrefix() + info.getPropName(), info.getAliasForm().toPropertyOptions());
/*     */ 
...
/*     */ 
/*     */               }
/* 377 */               else if (info.getAliasForm().isSimple())
/*     */               {
...
/* 481 */       if ((binGPSStamp.getYear() != 0) || (binGPSStamp.getMonth() != 0) || (binGPSStamp.getDay() != 0))
/*     */       {
/* 485 */         return;
/*     */       }
/*     */ 
...
/*     */   {
/* 521 */     for (Iterator it = tree.iterateChildren(); it.hasNext(); )
/*     */     {
/* 523 */       XMPNode schema = (XMPNode)it.next();
...
/*     */     throws XMPException
/*     */   {
/* 545 */     if ((!aliasNode.getValue().equals(baseNode.getValue())) || (aliasNode.getChildrenLength() != baseNode.getChildrenLength()))
/*     */     {
/* 548 */       throw new XMPException("Mismatch between alias and base nodes", 203);
/*     */     }
/*     */ 
/* 551 */     if ((!outerCall) && ((!aliasNode.getName().equals(baseNode.getName())) || (!aliasNode.getOptions().equals(baseNode.getOptions())) || (aliasNode.getQualifierLength() != baseNode.getQualifierLength())))
/*     */     {
/* 558 */       throw new XMPException("Mismatch between alias and base nodes", 203);
/*     */     }
/*     */ 
/* 562 */     Iterator an = aliasNode.iterateChildren();
/* 563 */     Iterator bn = baseNode.iterateChildren();
/* 564 */     while ((an.hasNext()) && (bn.hasNext()))
/*     */     {
/* 566 */       XMPNode aliasChild = (XMPNode)an.next();
/* 567 */       XMPNode baseChild = (XMPNode)bn.next();
/* 568 */       compareAliasedSubtrees(aliasChild, baseChild, false);
/*     */     }
/*     */ 
/* 572 */     Iterator an = aliasNode.iterateQualifier();
/* 573 */     Iterator bn = baseNode.iterateQualifier();
/* 574 */     while ((an.hasNext()) && (bn.hasNext()))
/*     */     {
/* 576 */       XMPNode aliasQual = (XMPNode)an.next();
/* 577 */       XMPNode baseQual = (XMPNode)bn.next();
...
/*     */     try
/*     */     {
/* 615 */       XMPNode dcSchema = XMPNodeUtils.findSchemaNode(((XMPMetaImpl)xmp).getRoot(), "http://purl.org/dc/elements/1.1/", true);
/*     */ 
 ...
 * Qualified Name:     com.itextpdf.xmp.impl.XMPNormalizer
